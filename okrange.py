...
def startyourengine():
    ...
    if len(tmp) == 1:
        ...
        arr -= np.max(arr)
        reduceby = getMaxInRange(freq, arr, float(startfreq), float(endfreq))
    ...
    if len(tmp) == 2:
        ...
        arr1 -= np.max(arr1)
        arr2 -= np.max(arr2)
        max1 = getMaxInRange(freq, arr1, float(startfreq), float(endfreq))
        max2 = getMaxInRange(freq, arr2, float(startfreq), float(endfreq))
